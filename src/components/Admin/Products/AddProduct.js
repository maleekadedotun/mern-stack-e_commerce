...
export default function AddProduct() {
  ...
  // files fileHandleChange
  const fileHandleChange = (event) => {
    const newFiles = Array.from(event.target.files);
    // file validation
    const newErr = []  // for react to update it
    newFiles.forEach((file) => {
      if (file?.size > 1000000) {
        newErr.push(`${file?.name} is too large`)
      }
      if (!file?.type.startsWith("image/")) {
        newErr.push(`${file?.name} must be an image`)
      }
    });
    setFiles(newFiles); 
    setFileError(newErr);
  ...
  return (
    <>
      ...
      {/* {fileError && <ErrorMsg message="File too large or upload an image" />} */}
      {fileError.length > 0 && <ErrorMsg message="File too large or upload an image" />}
